...
- `ScorePage.tsx` has two input modes (a segmented `.mode-toggle`, styled like the nav's `is-active` treatment): paste-a-vector (unchanged) and look-up-a-CVE. The CVE mode shows a cached/fresh indicator with a Refresh action, and — when NVD returns more than one disagreeing CVSS entry (per v1 spec §7) — a picker built from the interview page's existing `.option-card` selectable-card pattern rather than inventing a new one. Both modes render through the shared `ScoreResult` component.
...
Design system lives in `web/src/styles.css` as CSS custom properties (no framework, per v1 spec §3): IBM Plex Mono for headlines/scores/vector strings, Archivo for body copy (via `@fontsource/*`, self-hosted — **never** load fonts from a CDN, the spec requires the app work fully offline; SPEC1.md reaffirms the no-CDN rule). Import only the `latin-*.css` subset files from `@fontsource` packages, not the bare weight files — the latter pull in every unicode subset (cyrillic, vietnamese, greek, ...) and roughly quadruple the font payload for no reason in an English-only UI.
...
Built on `ae-cvss-calculator` (metaeffekt, Apache-2.0, zero runtime deps, covers CVSS 2.0/3.0/3.1/4.0) after evaluating it per v1 spec §2.4 — reproduces the spec's worked example (base `9.8` → environmental `0.0`) and known reference vectors exactly; see `server/test/scoring.test.ts`. It's a CommonJS package under `"type": "module"` — import it with `import pkg from "ae-cvss-calculator"; const { Cvss3P1, Cvss4P0 } = pkg;`, not named imports (TS's `NodeNext` resolution will reject named imports from a CJS package).
...
- Published image: `docker.io/eengelking/localscore` (tags `latest`, `0.1.0`) — pushed with `podman push`. v1 spec §9's example commands reference `ghcr.io/<owner>/localscore`; the maintainer chose Docker Hub instead, so README.md documents `docker.io/eengelking/localscore` as the real, working registry path. See "Publishing a release image" below for the required tag/push/verify procedure — never push an untagged or `latest`-only build.
...
**`SPEC1.md` is the active implementation contract.** Read it in its entirety before writing any code — it specifies the v1.1 scope: the design-system/button/icon overhaul, light/dark theme toggle, markdown description fields, environment and saved-vulnerability edit views, the saved-vulnerability dedup/overwrite fix, the Major CVEs feed, and the carried-over v1 gaps, plus the data-model/API changes and testing requirements for all of it. Treat every MUST/MUST NOT in it as a hard requirement and every SHOULD as the default unless there is a documented reason to deviate. Do not improvise architecture that SPEC1.md already decided.

The original v1 contract (SPEC.md) is fully implemented and no longer drives work — "v1 spec §N" citations throughout this file refer to its section numbers as historical rationale for existing code. SPEC1.md restates which v1 invariants remain binding (scoring parity, cap/override semantics, offline operation, the mandated tech stack).
...
The **question catalog lives in code** (a versioned TS module), not in the database — see v1 spec §5.1 for the `Question`/`Option`/`MetricEffect` shape. Each answer option emits zero or more `MetricEffect`s tagged with an `effect` of `override` or `cap`:
...
- Full metric-to-question mapping is in Appendix A of the v1 spec — consult it rather than re-deriving which question feeds which metric.
