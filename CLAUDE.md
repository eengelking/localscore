...
Each changed-metric line in the "why" panel (`ScoreChanges.tsx`) shows a colored ▲/▼ (reusing the `--severity-critical`/`--severity-low` tokens, same as `.delta-up`/`.delta-down`) sourced from the server's `ScoreChange.direction` field — exact per-metric numeric attribution isn't well-defined for CVSS (the formula isn't additive), so this is a directional signal only, not a magnitude. The severity pill (in both `AnimatedScore` and the base-score-card) sits in a fixed-width `.severity-slot` sized to the longest severity word ("CRITICAL") so score figures stay aligned in a column down a result list regardless of which severity word appears per row — `.pill-none`'s chrome-less rendering still fits the same slot.
...
- `server/src/scoring/orderings.ts` — the §2.2 severity orderings (covers every scoring-relevant metric, not just the cap-eligible subset), `baseCounterpartMetric()` (strip a leading `M` — works uniformly for every M-prefixed metric, cap or override; CR/IR/AR and the v4 supplemental metrics have no base counterpart), and `severityDirection()` (whether a change made a metric more/less/equally severe than the base vector, powering the ▲/▼ indicators in the "why" panel). CR/IR/AR treat a "Not Defined" `fromValue` as equivalent to `H` — CVSS scores an unspecified Security Requirement as High by default, so setting it explicitly to `H` is a no-op ("neutral"), not an increase. Metrics with no ordering entry (the v4.0 supplemental S/R/V/RE) always resolve "neutral" rather than throwing, since they're collected but never affect score.
