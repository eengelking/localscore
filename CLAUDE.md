...
**docs/SPEC02.md v1.1 progress**: §7.1 (saved-vs-cache double-entry fix), §2/§3 (design-system foundation: semantic buttons/icons/confirmation modal, light/dark theme toggle), and §4 (markdown description fields) are done — see "Design system" and "Markdown description fields" under Frontend below. Still open: §6.2/§6.4/§6.5 Scoring page tabs/offline/Major-CVEs, §7.2/§7.3/§7.4 saved-vulnerability editing/NVD-carry-through/result presentation, §8 environments edit view.
...
`web/src/` — `App.tsx` does simple state-based view switching (`environments` / `interview` / `score` / `vulnerabilities`), no router library. `pages/` holds the four screens, `components/` the shared pieces (`SeverityPill`, `AnimatedScore`, `ScoreChanges`, `EnvironmentResultRow`, `ScoreResult`, `NvdVectorPicker`, `Icon`, `Modal`, `ThemeToggle`). `api.ts` wraps every `/api/*` call; `types.ts` mirrors the server's response shapes.

- `ScorePage.tsx` has two input modes (a segmented `.mode-toggle`, styled like the nav's `is-active` treatment): paste-a-vector (unchanged) and look-up-a-CVE. The CVE mode shows a cached/fresh indicator with a Refresh action, and — when NVD returns more than one disagreeing CVSS entry (per docs/SPEC01.md §7) — a picker built from the interview page's existing `.option-card` selectable-card pattern rather than inventing a new one. Both modes render through the shared `ScoreResult` component.
...
- **Known gap, tracked for closure in docs/SPEC02.md §7.3**: the client doesn't send `nvdJson` when saving from a CVE lookup (the route accepts it), so a vulnerability saved from a CVE lookup won't show the `NvdVectorPicker` on its saved-vulnerabilities detail view until that CVE ID is separately re-looked-up through the Score page (which is what refreshes the `nvd_json` cache column).
...
- Currently wired up at the one place a description renders today: `EnvironmentsPage.tsx`'s list rows (`env.description`). No environment-description *editing* UI exists yet (tracked separately as §8.3's environment edit view) and saved-vulnerabilities have no `description` column yet (§7.2) — both will reuse `MarkdownContent` once built, not a new renderer.
...
- Out of scope for this pass (tracked separately): §4 markdown descriptions, §6 Scoring-page tabs/offline/Major-CVEs, §7.2–7.4 saved-vulnerability editing and result-presentation polish, §8 environment edit view. `.badge`/`.badge-severity` were audited against §2.5 and already share one box model (shape/size/weight), differing only in color — no change was needed there.
...
**`docs/SPEC02.md` is the active implementation contract.** Read it in its entirety before writing any code — it specifies the v1.1 scope: the design-system/button/icon overhaul, light/dark theme toggle, markdown description fields, environment and saved-vulnerability edit views, the saved-vulnerability dedup/overwrite fix, the Major CVEs feed, and the carried-over v1 gaps, plus the data-model/API changes and testing requirements for all of it. Treat every MUST/MUST NOT in it as a hard requirement and every SHOULD as the default unless there is a documented reason to deviate. Do not improvise architecture that docs/SPEC02.md already decided.

The original v1 contract lives at `docs/SPEC01.md` — fully implemented, it no longer drives work; "docs/SPEC01.md §N" citations throughout this file and the code are historical rationale for existing behavior. docs/SPEC02.md restates which v1 invariants remain binding (scoring parity, cap/override semantics, offline operation, the mandated tech stack). Future spec rounds continue the numbering in `docs/` (SPEC03.md, …).
